              ...
              {col.contact ? (
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 3, flexWrap: 'wrap' }}>
                    ...
                    <button onClick={async (e) => { e.stopPropagation(); const modes = ['off', 'draft', 'auto'] as const; const currentIdx = modes.indexOf((col.aiMode || 'off') as typeof modes[number]); const nextMode = modes[(currentIdx + 1) % modes.length]; if (nextMode === 'auto') { const confirmed = window.confirm(`⚠️ Enable AI Auto mode for ${col.contact?.name || 'this conversation'}?\n\nAuto mode will:\n• Automatically draft AND send responses\n• No approval needed — messages send immediately\n• Uses credits for each AI-sent message ($0.01/msg)\n\nAre you sure?`); if (!confirmed) return; } setColumns(prev => prev.map(c => c.id === col.id ? { ...c, aiMode: nextMode } : c)); setAllConversations(prev => prev.map(c => c.id === col.id ? { ...c, aiMode: nextMode } : c)); const convId = col.conversationId || col.id.replace('real-', ''); if (convId) { await supabase.from('conversations').update({ ai_mode: nextMode }).eq('id', convId); } }}
